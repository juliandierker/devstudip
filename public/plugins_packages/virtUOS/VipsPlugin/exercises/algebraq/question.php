...
/*
defined('MOODLE_INTERNAL') || die(); // #MOODLE

require_once($CFG->dirroot . '/question/type/algebra/questiontype.php'); // # MOODLE
require_once($CFG->dirroot . '/question/type/algebra/parser.php'); // # MOODLE
require_once($CFG->dirroot . '/question/type/algebra/xmlrpc-utils.php'); // # MOODLE
*/

const MOODLE_INTERNAL = true;
# require_once('questiontype.php'); // #TT
require_once('parser.php'); // #TT
//require_once('xmlrpc-utils.php');

/*
 * Moodle throws error whenever it cannot parse an expression. The get_string function
 * is not defined in this package so there will be an error without this function.
 */
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
//class qtype_algebra_question extends question_graded_by_strategy // #MOODLE
//        implements question_response_answer_comparer { // #MOODLE
class qtype_algebra_question {
    ...
    public function __construct() {
        // parent::__construct(new question_first_matching_answer_grading_strategy($this)); // #MOODLE
    }
    ...
    public function parse_expression($expr) {
        ...
        //var_dump("question.php: parse_expression");
        ...
        try {
            // return $p->parse($expr, $varnames); // #MOODLE
            //////var_dump($p->parse($expr, $varnames, true));
            return $p->parse($expr, $varnames, true); //#noesting
            //return $p->parse($expr, $varnames, false);
        } catch (Exception $e) {
    ...
    public function is_same_response(array $prevresponse, array $newresponse) {
        ...
        //TODO get variables from tree, append in variables and set default min max
        
//        echo "First expression: ---------------";
//        //var_dump($expr->str());
//        echo "Second expression: ---------------";
//        //var_dump($testexpr->str());
        
        if($expr->str() == null || $testexpr->str() == null) {
            return false;
        }
        
        foreach($expr->get_variables() as $vars) {
            $var = new stdClass();
            $var->name = $vars;
            $var->min = -100;
            $var->max = 100;
        ...
        foreach($testexpr->get_variables() as $vars) {
            $var = new stdClass();
            $var->name = $vars;
            $var->min = -100;
            $var->max = 100;
...
//        $request['args'] = [$answer->sage(), $response->sage(), $vars];
...
//    }
    ...
    public function test_response_by_evaluation($response, $answer) {
        //var_dump("question.php: response_by_evaluation");
        ...
        for ($i = 0; $i < $this->nchecks; $i++) {
            ...
            //var_dump("question.php: got evaluation 1");
            $ansvalue = $answer->evaluate($values);
            //var_dump("question.php: got evaluation 2");
            //echo "<p>Werte: "; print_r($values); //echo " - Ergebnisse: $respvalue ; $ansvalue"; // TT: Debug
            ...
            ////var_dump("ansvalue: ".$ansvalue);
            ////var_dump("respvalue: ".$respvalue);
            if (abs($ansvalue) > 1e-300) {
                //var_dump(abs($ansvalue));
            ...
            if ($diff > $this->tolerance) {
                //var_dump("diff: ".$diff);
                //var_dump("tolerance: ".$this->tolerance);
                //die;
        ...
        //die;
        //var_dump("question.php: response_by_evaluation finished");
        return true;
    }
